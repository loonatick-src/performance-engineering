...
std::vector<double> BM_rand(size_t count) {
    auto seedp = &seed;
    std::vector<double> t_iter;
    double x;
    PE_LOOP {
        START_TIMER;
        x = random_double_r(seedp);
        END_TIMER;
...
std::vector<double> BM_rand_dup(size_t count) {
    auto seedp = &seed;
    std::vector<double> t_iter;
    double x;
    PE_LOOP {
...
std::vector<double> BM_rand_ab(size_t count) {
    auto seedp = &seed;
    std::vector<double> t_iter;
    double x;
    PE_LOOP {
...
int main(void) {
    ...
    bench_log(name);
    PE_BENCH(BM_rand, name);
    bench_log(name);
    PE_BENCH(BM_rand_dup, name);

    name = std::string("rand_ab");
    bench_log(name);
    PE_BENCH(BM_rand_ab, name);
